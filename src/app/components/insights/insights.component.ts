...
export class InsightsComponent {
        ...
        datasets: [
          {
            label: 'Apple',
            data: [478, 356, 492, 345, 763, 883, 672, 654, 439, 888, 991, 567],
            backgroundColor: 'rgba(255, 99, 132, 0.9)', // Light Red
           borderRadius:4
          },
          {
            label: 'Samsung',
            data: [530, 843, 672, 934, 411, 556, 692, 772, 493, 605, 859, 726],
            backgroundColor: 'rgba(54, 162, 235, 0.9)', // Light Blue
           borderRadius:4
          },
          {
            label: 'Sony',
            data: [625, 934, 495, 813, 251, 912, 653, 701, 823, 457, 609, 957],
            backgroundColor: 'rgba(153, 102, 255, 0.9)', // Light Purple
           borderRadius:4
          },
        ],
      },
  ...
  updateChart(event: any): void {
    ...
    if (this.lineChart) {
      ...
      if (this.selectedFilter === 'brand') {
        ...
        this.lineChart.options.plugins!.title!.text =
          'Number of Orders : Brand Performance Over Months';
      } else if (this.selectedFilter === 'state') {
        this.lineChart.data.datasets = this.stateDatasets;
        this.lineChart.options.plugins!.title!.text =
          'Number of Orders : State Performance Over Months';
      } else if (this.selectedFilter === 'retailer') {
        this.lineChart.data.datasets = this.retailerDatasets;
        this.lineChart.options.plugins!.title!.text =
          'Number of Orders : Retailer Performance Over Months';
      }
  ...
  updateChartDollarVolume(event: any): void {
    ...
    if (this.lineChartDollarVolume) {
      ...
      if (this.selectedFilter === 'brand') {
        this.lineChartDollarVolume.data.datasets =
          this.brandDatasetsDollarVolume;
        this.lineChartDollarVolume.options.plugins!.title!.text =
          'Dollar Volume : Brand Performance Over Months';
      } else if (this.selectedFilter === 'state') {
        this.lineChartDollarVolume.data.datasets =
          this.stateDatasetsDollarVolume;
        this.lineChartDollarVolume.options.plugins!.title!.text =
          'Dollar Volume : State Performance Over Months';
      } else if (this.selectedFilter === 'retailer') {
        this.lineChartDollarVolume.data.datasets =
          this.retailerDatasetsDollarVolume;
        this.lineChartDollarVolume.options.plugins!.title!.text =
          'Dollar Volume : Retailer Performance Over Months';
      }
